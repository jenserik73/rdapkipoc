    ...
    with conn.cursor() as cur:
        ...
        try:
            cur.execute(
                ...
                VALUES (:uid, :pid, :gby)
                """,
                uid=user_id, pid=profile_id, gby=granted_by,
            )
...
def _revoke_access(conn: oracledb.Connection, profile_id: str, user_id: str, ctx) -> fdk.response.Response:
    with conn.cursor() as cur:
        cur.execute(
            "DELETE FROM qc_user_ai_profiles WHERE user_id = :uid AND ai_profile_id = :pid",
            uid=user_id, pid=profile_id,
        )
        # Hvis dette var brukerens aktive profil, fall tilbake til default
        cur.execute(
            "SELECT ai_profile_id FROM qc_user_active_ai_profile WHERE user_id = :uid",
            uid=user_id,
        )
...
def _list_my_profiles(conn: oracledb.Connection, user_id: str, ctx) -> fdk.response.Response:
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE  up.user_id = :uid AND p.is_active = 'Y'
...
def _set_my_active_profile(conn: oracledb.Connection, user_id: str, body: dict, ctx) -> fdk.response.Response:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE user_id = :uid AND ai_profile_id = :pid
            """,
            uid=user_id, pid=profile_id,
        )
        if cur.fetchone()[0] == 0:
        ...
        cur.execute(
            ...
            WHEN NOT MATCHED THEN
                INSERT (user_id, ai_profile_id) VALUES (:uid, :pid)
            """,
            uid=user_id, pid=profile_id,
        )
    conn.commit()
